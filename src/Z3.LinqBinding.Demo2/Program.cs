	...
	internal class Program
	{
		...
		private static void TestSudoku()
		{
			...
			using (var ctx = new Z3Context())
			{
				...
				var theorem = SudokuAsArray
					.Parse(
						"9.2..54.31...63.255.84.7.6..263.9..1.57.1.29..9.67.53.24.53.6..7.52..3.4.8..4195.") // Very easy
					.CreateTheorem(ctx);
				var result = theorem.Solve();
				Console.WriteLine(result);
				...
				theorem = SudokuAsArray
					.Parse(
						"..6.......8..542...4..9..7...79..3......8.4..6.....1..2.3.67981...5...4.478319562") // Medium
					.CreateTheorem(ctx);
				result = theorem.Solve();
				Console.WriteLine(result);
				theorem = SudokuAsArray
		...
		private static void TestMissionariesAndCannibals()
		{
			...
			using (var ctx = new Z3Context())
			{
				...
				var endTime = stopwatch.Elapsed;
				Console.WriteLine(
					"Minimal Solution to missionaries and cannibals non simplified through Z3 optimization");
				Console.WriteLine($"Time to solve: {endTime - startTime}");
