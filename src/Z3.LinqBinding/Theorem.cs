...
namespace Z3.LinqBinding
{
	...
	public class Theorem
	{
		...
		private object ConvertZ3Expression(object destinationObject, Context context, Model model, Environment subEnv, PropertyInfo parameter)
		{
			...
			//
			if (parameterTypeMapping == null) return value;
			var ctor = parameter.PropertyType.GetConstructor(new Type[] { parameterType }) ?? throw new InvalidOperationException("Could not construct an instance of the mapped type " + parameter.PropertyType.Name + ". No public constructor with parameter type " + parameterType + " found.");
